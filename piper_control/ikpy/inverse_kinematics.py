...
def inverse_kinematic_optimization(chain, target_frame, starting_nodes_angles, regularization_parameter=None, max_iter=None, orientation_mode=None, no_position=False, optimizer="least_squares"):
    ...
    optimizer: str
        ...
        * "fmin_slsqp": Use scipy.optimize.fmin_slsqp with enhanced precision
        * "trust_constr": Use scipy.optimize.minimize with trust-constr method
        * "lbfgsb": Use scipy.optimize.minimize with L-BFGS-B method
        * "multi_start": Use multiple random starting points with best optimizer
    """
    if optimizer not in ["least_squares", "scalar", "fmin_slsqp", "trust_constr", "lbfgsb", "multi_start"]:
    ...
    if optimizer == "scalar":
    ...
    elif optimizer == "fmin_slsqp":
        def optimize_scalar(x):
            return np.linalg.norm(optimize_total(x))
        
        # fmin_slsqp expects bounds in a different format: list of (min, max) tuples
        bounds_list = [(bound[0], bound[1]) for bound in real_bounds]
        
        # Enhanced precision settings for fmin_slsqp
        solution = scipy.optimize.fmin_slsqp(
            optimize_scalar,
            chain.active_from_full(starting_nodes_angles),
            bounds=bounds_list,
            acc=1e-8,  # Higher precision
            iter=1000,  # More iterations
            iprint=0  # Suppress output
        )
        
        # Create a result object similar to other optimizers
        class FminSlsqpResult:
            def __init__(self, x, success=True):
                self.x = x
                self.status = 0 if success else -1
                self.success = success
        
        res = FminSlsqpResult(solution, success=True)
        
    elif optimizer == "trust_constr":
        def optimize_scalar(x):
            return np.linalg.norm(optimize_total(x))
        
        # trust-constr method with enhanced settings
        bounds = scipy.optimize.Bounds([bound[0] for bound in real_bounds], 
                                      [bound[1] for bound in real_bounds])
        ...
        # L-BFGS-B method with enhanced settings
        bounds_list = [(bound[0], bound[1]) for bound in real_bounds]
    ...
    elif optimizer == "multi_start":
        def optimize_scalar(x):
            return np.linalg.norm(optimize_total(x))
        
        # Multi-start optimization with different starting points
        bounds_list = [(bound[0], bound[1]) for bound in real_bounds]
        best_result = None
        best_cost = float('inf')
        ...
        for i in range(num_starts):
            if i == 0:
                # First attempt with provided starting point
                x0 = chain.active_from_full(starting_nodes_angles)
            else:
            ...
            try:
                # Use L-BFGS-B for each starting point
                current_res = scipy.optimize.minimize(
                    optimize_scalar,
                    x0,
                    method='L-BFGS-B',
                    bounds=bounds_list,
                ...
                if current_res.success and current_res.fun < best_cost:
                    best_cost = current_res.fun
                    best_result = current_res
                    
            except Exception as e:
                logs.logger.warning("Multi-start attempt {} failed: {}".format(i, e))
                continue
        
        if best_result is not None:
            res = best_result
        else:
            # Fallback to standard method if all multi-start attempts fail
            res = scipy.optimize.minimize(
                optimize_scalar,
                chain.active_from_full(starting_nodes_angles),
                method='L-BFGS-B',
