...
class FrankaController(BaseRobotController):
    """IKPy-based controller for the 7-DOF Franka Emika Panda arm and gripper."""
    ...
    def __init__(
        ...
        urdf_path: str = "robots/franka_control/franka_panda.urdf",
        ...
        self.kinematics = FrankaKinematics(model, urdf_path, prefix=self.prefix)
        self.home_joints = np.array([0.0, 0.0, 0.0, -1.57079, 0.0, 1.57079, -0.7853], dtype=float)
        self.open_gripper_ctrl = 255.0
        self.closed_gripper_ctrl = 0.0
        self.open_gripper_width = 0.04
        self.closed_gripper_width = 0.0
        self._reset_to_home()

    def get_robot_state(self) -> FrankaState:
        ...
        r_world_tool = r_base * R.from_quat(q_local)
    ...
    def format_state(self) -> str:
        ...
        lines.append(f"  Gripper Width: {state.gripper_width:.4f}")
        return "\n".join(lines)
    ...
    def action_move_cartesian(
        ...
        **kwargs,
        ...
        r_local = r_base.inv() * r_world_tool
    ...
    def action_move_linear(
        ...
        **kwargs,
        ...
        r_local_tool_start = R.from_quat(ee_pose_local_start[3:])
        ...
        target_pose_local_end = np.concatenate([target_pos_local, r_local_tool_end.as_quat()])
        ...
        for step in range(1, total_steps + 1):
            ...
            if step == 1 or step % ik_step_skip == 0 or step == total_steps:
                ...
                target_pose_local = np.concatenate([interp_pos_local, r_local_tool_interp.as_quat()])
    ...
    def action_open_gripper(self):
        self._move_gripper(self.open_gripper_ctrl, self.open_gripper_width)

    def action_close_gripper(self):
        self._move_gripper(self.closed_gripper_ctrl, self.closed_gripper_width)
    ...
    def _reset_to_home(self):
        ...
        self._set_gripper_direct(self.open_gripper_width)
        self._update_ctrl(self.home_joints)
        self._set_gripper_ctrl(self.open_gripper_ctrl)
    ...
    def _set_gripper_direct(self, width: float):
        for finger_joint_id in self.kinematics.finger_joint_ids:
            if finger_joint_id != -1:
                self.data.qpos[self.model.jnt_qposadr[finger_joint_id]] = width

    def _set_gripper_ctrl(self, ctrl_value: float):
        if self.kinematics.gripper_actuator_id != -1:
            self.data.ctrl[self.kinematics.gripper_actuator_id] = ctrl_value
    ...
    def _move_gripper(self, target_ctrl: float, target_width: float, timeout: float = 2.0):
        self._set_gripper_ctrl(target_ctrl)
        sim_start_time = self.data.time
    ...
    def _get_gripper_width(self) -> float:
        if self.kinematics.finger_joint_ids[0] == -1:
            return 0.0
        qpos_adr = self.model.jnt_qposadr[self.kinematics.finger_joint_ids[0]]
...
class FrankaKinematics:
    ...
    def __init__(self, model: mujoco.MjModel, urdf_path: str, prefix: str = ""):
        ...
        self.joint_names = [f"{p}joint{i}" for i in range(1, 8)]
        self.joint_ids = [mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, name) for name in self.joint_names]
        self.actuator_ids = [
            mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_ACTUATOR, f"{p}actuator{i}") for i in range(1, 8)
        ]
        self.gripper_actuator_id = mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_ACTUATOR, f"{p}actuator8")
        self.finger_joint_ids = [
            mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, f"{p}finger_joint1"),
            mujoco.mj_name2id(model, mujoco.mjtObj.mjOBJ_JOINT, f"{p}finger_joint2"),
        ]

        root = ET.parse(urdf_path).getroot()
        urdf_joint_names = [joint.get("name") for joint in root.findall("joint")]
        active_links_mask = [False] * (len(urdf_joint_names) + 1)
        self.arm_link_indices = []
        limits = []

        for idx, name in enumerate(urdf_joint_names, start=1):
            if name and name.startswith("panda_joint") and name[11:].isdigit() and 1 <= int(name[11:]) <= 7:
                active_links_mask[idx] = True
                self.arm_link_indices.append(idx)
                joint = root.findall("joint")[idx - 1]
    ...
    def _full_joints(self, joints: np.ndarray) -> np.ndarray:
        ...
        for idx, joint_val in zip(self.arm_link_indices, joints):
            full[idx] = joint_val
        return full
    ...
    def inverse_kinematics(
        ...
        return np.array([best_sol[idx] for idx in self.arm_link_indices], dtype=float)
